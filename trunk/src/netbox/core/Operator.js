...
/** Implements the operator class.
  ...
  */
Ext.define('Ext.ux.netbox.core.Operator', {
    ...
    if(values !==null && values !== undefined && Ext.type(values)=="array"){
      if(values.length>0 && values[0]!== undefined){
        if(values.length==1){
          ...
          if (this.isMultipleValues()) return values;
          return([values[0]]);
        }
      }
    }
  ...
  isMultipleValues: function() {
	  return this.isRange() || this.id.substring(this.id.length-"LIST".length)=="LIST";
  },
  
  isRange: function() {
	  return this.id.substring(this.id.length-"RANGE".length)=="RANGE";
  }
